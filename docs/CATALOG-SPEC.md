...
- **Say a hole is threaded** and get taps and drills as two tabs; the predrill
  control over the drills says whether the thread is cut or rolled, and the
  drills are judged against whichever it is.
...
- `apps/catalog/app/client/` — the matching worker, off the UI thread
...
- `docs/PART-TOOL-TABLE-PLAN.md` — replacing this table with the kit's `Table`
...
> - ~~**Should the drawing show the feature?**~~ **Answered 2026-09-03** by
>   Justin: the panel draws the part the tool is read for. The page had the
>   feature's reach curve in hand the whole time and spent it only on the holder
>   list, so the one place a shop reads a tool showed no reason for the stickout
>   the list beside it had settled on.
...
- `packages/catalog-data/src/stickout.ts` — one module owns how far a tool stands out
- `docs/HOLDER-PROFILES.md` — holders drawn from their own CAD rather than nine published numbers
...
- **Two words, not two figures** (Paul, 2026-09-07: "don't show the numbers,
  just tap or form drill"). The chart size and the difference from the model
  were printed on the control and again, per row, in the list underneath — which
  is where a shop reads a drill's deviation, in the table's own columns. They are
  on the hover now. What survives on the control is the one thing the list cannot
...
- `app/shared/catalog-matcher.ts` — the one matching truth, shared by the table and the rows
- `app/client/catalog-matcher.ts` · `.worker.ts` — the same work, off the UI thread
- `docs/CATALOG-MATCHING-PERFORMANCE-PLAN.md` — why it is a worker
