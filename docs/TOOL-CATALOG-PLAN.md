...
- **The part page is a feature list** since 2026-09-02, and the list drives
  ...
  before touching that page, with `docs/CATALOG-SPEC.md` over the top of it.
...
| Narrowing a number cost four presses before a keystroke, and the operator list was a popover inside a popover                                                | The cost was in the shape of the control, and no test asks a shape       | The control has no operator list: `shared/range-entry.test.ts`          |
| Clearing a bound took the filter off and left the rules that wrote it judging, while the tick that sets them aside was offered only with a number in the box | The filter and the rules were tested apart, and the gap was between them | `shared/filter.test.ts` (`releasedBounds`), `tests/on-the-part.spec.ts` |
