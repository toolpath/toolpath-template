...
/**
 * How wide the panel over the part is, and what a shop may drag it to.
 *
 ...
 * and expanding to the right"). One panel carries all four of those — the three
 ...
 * where a panel ends up wider than the window it stands in.
 *
 ...
 * groups: once a shop has said how wide this panel is, that is how wide it is
 * whatever is in it. A panel that jumped 96px every time a group opened would
 * be undoing the drag that set it.
 */

/** Where the width is remembered: the shop's, in this browser, not the part's. */
const KEY = 'tool-catalog.panel-width'
 ...
 * where the panel is taller than the viewer and says less, which is a worse
 * screen than the part being covered — and there is a fold button for that.
 */
export const NARROWEST = 256

/** What the panel opens at, never having been dragged: today's `w-80`. */
...
/**
 * The most of the viewer the panel may take.
 *
 ...
 *
 * **It is a ceiling on the drag, not on the panel.** The handle measures the
 ...
 * a circle: the overlay the panel stands in is shrink-to-fit, so a percentage
 * `max-width` resolves against the panel's own width and squeezes it to a share
...
/**
 * The widest the panel may be dragged, given the viewer it stands over.
 *
 ...
 */
export const widestPanel = (room: number): number => {
...
export const clampPanel = (wanted: number, room: number): number => {
  if (!Number.isFinite(wanted)) {
    return OPENS_AT
  }
  return Math.min(Math.max(wanted, NARROWEST), widestPanel(room))
}

/** What the panel is drawn at: what the shop stated, or what the box opens at. */
export const panelWidth = (stated: number | null, buildingAGroup: boolean): number => {
  if (stated === null) {
 ...
 * not walk on it.
 */
export const readPanelWidth = (storage: Pick<Storage, 'getItem'> | null): number | null => {
