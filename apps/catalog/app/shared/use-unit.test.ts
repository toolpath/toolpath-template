 ...
 *
 * The DFM application has had this since its own move; the catalog went through
 * the same rename with none of it, which is the gap this closes.
 */
describe('the unit this application remembers', () => {
  it('is still keyed where it has always been', () => {
    expect(UNIT_STORAGE_KEY).toBe('tool-catalog.unit')
  })
  ...
  it('keeps the DFM application’s preference out of this one', () => {
    const { storage } = store()
    saveUnit(storage, 'part-viewer.unit', 'inches')
